app_name = "frappe_ai"
app_title = "Frappe AI"
app_publisher = "Sambhav Saxena"
app_description = "A central command for managing all AI operations inside Frappe Framework."
app_email = "sambhavsaxena02@gmail.com"
app_license = "mit"
...
app_include_js = [
	"/assets/frappe_ai/js/agent_chat.js",
	# "/assets/frappe_ai/js/agent_launcher.js",
]
...
boot_session = "frappe_ai.boot.boot_session"

# Refresh the process-level client registry when an AI Client doc is saved.
doc_events = {
	"AI Client": {
		"on_update": "frappe_ai.clients.refresh_ai_client",
	}
}

scheduler_events = {
	# Poll GCP for completed async (batch) OCR operations every 5 minutes.
	"cron": {
		"*/5 * * * *": [
			"frappe_ai.document_ai.jobs.check_ocr_process_status",
		],
		# Compact FAISS indexes weekly (Sunday 03:00) to remove inactive chunks.
		"0 3 * * 0": [
			"frappe_ai.rag.compaction.compact_all_rag_indexes",
		],
		# Run GCP bucket cleanup function every 15 days.
		"0 0 */15 * *": [
			"frappe_ai.document_ai.jobs.delete_processed_ocr_data_from_gcs"
		]
	},
}
